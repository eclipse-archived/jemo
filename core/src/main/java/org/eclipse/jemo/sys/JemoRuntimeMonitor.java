...
public class JemoRuntimeMonitor {

    public static final String JEMO_MONITOR = "/jemo/monitor";
    private static final String JEMO_PLUGINS = JEMO_MONITOR + "/plugins";
        ...
        switch (request.getMethod()) {
            case "GET":
                if (JEMO_PLUGINS.equals(request.getRequestURI())) {
                    ...
                    loadFile(request.getRequestURI().replaceAll(JEMO_MONITOR, ""), response);
                }
                break;
            case "POST":
    ...
    private static void loadFile(String requestUri, HttpServletResponse response) throws IOException {
        ...
        final InputStream in = Jemo.class.getResourceAsStream("/monitor" + fileName);
