...
public class X2Module {
	...
	public X2Module(Module module,ModuleMetaData metaData) {
		this.module = module;
	...
	public static boolean implementsBatch(final Class<? extends Module> moduleClass) {
		return Optional.ofNullable(CC.executeFailsafe(x -> {
	...
	public static boolean implementsEvent(final Class<? extends Module> moduleClass) {
		return Optional.ofNullable(CC.executeFailsafe(x -> {
			return !moduleClass.getMethod("process", CCMessage.class).getDeclaringClass().getName().equals(Module.class.getName());
		}, null)).orElse(false);
	}
