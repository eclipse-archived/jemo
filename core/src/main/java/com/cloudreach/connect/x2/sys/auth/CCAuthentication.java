 ...
 * is part of.
 * A user will have an assigned password which must be used for authentication to X2.
 * Every time an operation is executed on the plugin manager this must be written on an audit trail log service.
 * 
 * @author christopher stura
 */
public class CCAuthentication {
	protected static final String TABLE_GROUPS = "cloudreach_x2_security_groups";
	protected static final String TABLE_USERS = "cloudreach_x2_security_users";
	...
	private static final String USER_ADMIN = "system.administrator@cloudreach.x2.com";
	
	public static void init(AbstractX2 x2server) throws NoSuchAlgorithmException, UnsupportedEncodingException, IOException {
		CCGroup adminGroup = null;
		if(SystemDB.createTable(TABLE_GROUPS)) {
			//we need to create the default system administrator group.
			adminGroup = new CCGroup("X2 Administrator", "Global Administration Group", ".*", 0, Integer.MAX_VALUE);
		...
		if(SystemDB.createTable(TABLE_USERS)) {
			...
			if(adminGroup == null) {
				adminGroup = SystemDB.get(TABLE_GROUPS, Util.md5(GROUP_ADMIN), CCGroup.class);
			}
			...
			x2server.LOG(Level.OFF,"System Authorisation Configured for the First Time: Admin username: %s Password: %s store this in a safe place as it will not be repeated", USER_ADMIN, adminPassword);
		}
		x2server.LOG(Level.INFO, "System Authentication/Authorisation Initialised successfully");
	}
	 ...
	 */
	public static List<CCGroup> listGroups() {
		return SystemDB.list(TABLE_GROUPS, CCGroup.class);
	}
	
	public static CCUser getDefaultAdminUser() throws IOException {
		return getUser(USER_ADMIN);
	}
	
	public static CCUser getUser(String username) throws IOException {
		return SystemDB.get(TABLE_USERS, username, CCUser.class);
	}
	...
	public static void processRequest(CCUser authUser,HttpServletRequest request, HttpServletResponse response) throws IOException,NoSuchAlgorithmException {
		if(authUser == null) {
		...
		if("/x2manager/authentication/user".equals(request.getRequestURI())) {
			if("POST".equals(request.getMethod())) {
				//we are going to be upserting a user
				CCUser user = Util.fromJSONString(CCUser.class, Util.toString(request.getInputStream()));
				...
				CCUser existingUser = getUser(user.getUsername());
				...
				CCUser delUser = getUser(userItem);
				if(delUser == null) {
				...
				List<CCUser> userList = SystemDB.list(TABLE_USERS, CCUser.class);
		...
		} else if("/x2manager/authentication/group".equals(request.getRequestURI())) {
			if("POST".equals(request.getMethod())) {
				//we need to check if this group already exists.
				CCGroup group = Util.fromJSONString(CCGroup.class, Util.toString(request.getInputStream()));
				if(group != null) {
					CCGroup existingGroup = SystemDB.get(TABLE_GROUPS, group.getId(), CCGroup.class);
				...
				List<CCGroup> groupList = new ArrayList<>();
				if(groupItem != null) {
					CCGroup group = SystemDB.get(TABLE_GROUPS, groupItem, CCGroup.class);
					if(group != null) {
						groupList.add(group);
					} else {
				...
				if(userItem != null) {
					...
					CCUser user = SystemDB.get(TABLE_USERS, userItem, CCUser.class);
					if(user == null) {
