 ...
 */
public class X2ApplicationMetaData implements SystemDBObject {
	
	public static class X2ModuleFrequency extends Frequency {
		
		public X2ModuleFrequency() {
			super();
		}
		...
		public static X2ModuleFrequency wrap(Frequency frequency) {
			if(frequency != null) { 
				if(frequency instanceof X2ModuleFrequency) {
					return X2ModuleFrequency.class.cast(frequency);
				} else {
					X2ModuleFrequency retval = new X2ModuleFrequency();
	...
	public static class X2ModuleLimits extends ModuleLimit {
		...
		public void setBatchFrequency(X2ModuleFrequency batchFrequency) {
			this.batchFrequency = batchFrequency;
		}

		public void setEventFrequency(X2ModuleFrequency eventFrequency) {
		...
		public static X2ModuleLimits wrap(ModuleLimit limit) {
			X2ModuleLimits lim = new X2ModuleLimits();
			lim.setBatchFrequency(X2ModuleFrequency.wrap(limit.getBatchFrequency()));
			lim.setBatchLocations(limit.getBatchLocations());
			lim.setEventFrequency(X2ModuleFrequency.wrap(limit.getEventFrequency()));
	...
	private Map<String,X2ModuleLimits> limits = new ConcurrentHashMap<>();
	...
	public Map<String, X2ModuleLimits> getLimits() {
		return limits;
	}

	public void setLimits(Map<String, X2ModuleLimits> limits) {
		this.limits = limits;
	}
