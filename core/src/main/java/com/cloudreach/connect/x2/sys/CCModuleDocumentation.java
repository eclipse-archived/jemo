 ...
 */
public class CCModuleDocumentation {
	
	private Map<String,X2ClassLoader> docModuleCache = new HashMap<>();
	private final AbstractX2 x2server;
	
	public CCModuleDocumentation(AbstractX2 x2server) {
	...
	public synchronized X2ClassLoader getDocumentationClassLoader(String moduleJar) throws IOException {
		X2ClassLoader docClassLoader = null;
		Set<X2Module> moduleList = x2server.getPluginManager().getModulesByPluginId(CCPluginManager.PLUGIN_ID(moduleJar));
		if(moduleList != null && !moduleList.isEmpty()) {
			docClassLoader = X2ClassLoader.class.cast(moduleList.iterator().next().getModule().getClass().getClassLoader());
		} else if(docModuleCache.containsKey(moduleJar)) {
			return docModuleCache.get(moduleJar);
		} else {
			docClassLoader = x2server.getPluginManager().buildModuleClassLoader(moduleJar);
	...
	public void processRequest(CCUser user,HttpServletRequest request,HttpServletResponse response) throws IOException {
		if(user == null) {
			...
			int endModeName = request.getRequestURI().indexOf('/',"/x2manager/docs".length()+1);
			if(!(endModeName > "/x2manager/docs".length())) {
				response.sendRedirect(request.getRequestURI()+"/");
				return;
			}
			String modJar = endModeName > "/x2manager/docs".length() ? request.getRequestURI().substring("/x2manager/docs".length()+1,endModeName) : request.getRequestURI().substring("/x2manager/docs".length()+1);
			...
			//obviously we don't want to burden the memory with storing stuff just for the sake of documentation, so what we will do is build an X2ClassLoader to extract
			...
			X2ClassLoader docLoader = getDocumentationClassLoader(modJar);
			if(docLoader != null) {
				InputStream strIn = docLoader.getResourceAsStream(docPage);
				if(strIn != null) {
					CC.stream(response.getOutputStream(), strIn, true);
				} else {
