 ...
 */
public class CCScheduler extends Thread {
	...
	protected static class ModuleActivityMap {
		...
		private final CCPluginManager.ModuleInfo module;
		private final Set<String> instances;
		private final ModuleLimit limits;
		
		public ModuleActivityMap(AbstractX2 x2server, CCPluginManager.ModuleInfo module,Set<String> instances,ModuleLimit limits) {
			this.module = module;
		...
		public int getCurrentGSMActivity() {
			return x2server.getPluginManager().getNumModuleEventsRunningOnGSM(module.getId(), module.getVersion(), module.getImplementation());
		}
		
		public int getCurrentInstanceActivity(String instanceId) {
			return CCPluginManager.getNumModuleEventsRunning(instanceId, module.getId(), module.getVersion(), module.getImplementation());
		}
		
		public int getCurrentLocationActivity(String location) {
			return x2server.getPluginManager().getNumModuleEventsRunningOnLocation(location,module.getId(), module.getVersion(), module.getImplementation());
		}
		...
		public boolean isValidForFrequencyInterval() {
			...
			long lastExecutionDate = x2server.getPluginManager().getLastLaunchedModuleEventOnGSM(module.getId(), module.getVersion(), module.getImplementation());
	...
	private AbstractX2 x2server = null;
	...
	public CCScheduler(AbstractX2 x2server) {
		this.x2server = x2server;
	}
	...
	public Nomination getCurrentNomination() {
		...
		if(nominee != null && LAST_NOMINATION != null) {
			...
			if(timeSinceNomination > TimeUnit.MINUTES.toMillis(5)) { //the scheduler will hold it's nomination for 5 minutes (this will make X2 cheaper to run)
				//this means that the last nomination is stale because of the time in which the nomination was made.
				return null;
			} else if(!x2server.getPluginManager().isInstanceActive(nominee)) {
				return null;
			} else {
	...
	public Nomination newNomination() {
		...
		Set<String> activeInstanceList = x2server.getPluginManager().getActiveInstanceList();
		...
		if(nominatingInstanceId != null) { //if at least 1 instance is active in the GSM.
			if(nominatingInstanceId.equals(x2server.getINSTANCE_ID())) {
				...
				if(nominatedInstanceId.equals(x2server.getINSTANCE_ID())) {
					return nomination;
				}
			}
		}
	...
	public void interrupt() {
		...
		x2server.LOG(Level.INFO, "[%s][%s] THE SCHEDULER HAS BEEN INTERUPTED", getClass().getSimpleName(), x2server.getINSTANCE_ID());
	}
	...
	public void run() {
		while(RUNNING.get()) { //run as long as nobody has interrupted us.
			...
			if(currentNomination != null && currentNomination.getInstanceId().equals(x2server.getINSTANCE_ID())) {
				//1. we will need to know the module list for each active instance in the cluster.
				x2server.LOG(Level.INFO, "[%s][%s] NOMINATED SCHEDULER IS %s - RUNNING STATUS IS %s", getClass().getSimpleName(), x2server.getINSTANCE_ID(), currentNomination.getInstanceId(), String.valueOf(RUNNING.get()));
				//we need to know where things are being executed so we can filter out modules which are over their limits.
				Set<String> activeInstanceList = x2server.getPluginManager().getActiveInstanceList();
				Map<String,String> instanceLocationMap = x2server.getPluginManager().getInstanceLocationMap(activeInstanceList.toArray(new String[] {}));
				Map<String,List<CCPluginManager.ModuleInfo>> instanceModuleMap = activeInstanceList
					.stream()
					.map(inst -> new KeyValue<>(inst,
						Arrays.asList(x2server.getPluginManager().getModuleList(inst)).stream()
				...
				List<ModuleActivityMap> activityMap = instanceModuleMap.values().stream()
					.flatMap(List::stream)
					.map(m -> new ModuleActivityMap(x2server, m, activeInstanceList, x2server.getPluginManager().getModuleLimits(m.getId(), m.getVersion(),m.getImplementation())))
				...
				Map<CCPluginManager.ModuleInfo,Set<String>> moduleInstanceMap = instanceModuleMap.entrySet().stream()
				...
				moduleInstanceTargetMap.entrySet().stream()
					...
					if(queueUrl != null) {
						...
						if(moduleId != -1) {
							if(RUNNING.get()) {
								x2server.sendRunBatchMessage(moduleId, e.getValue(), queueUrl);
							}
						}
					}
				});
