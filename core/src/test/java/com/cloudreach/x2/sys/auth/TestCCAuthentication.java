...
********************************************************************************/
package com.cloudreach.x2.sys.auth;
...
/**
 * this test should cover the CCAuthentication class to make sure all of the features
 * offered through that web service implementation are covered.
 * 
 * @author Christopher Stura "christopher.stura@cloudreach.com"
 */
public class TestCCAuthentication extends X2BaseTest {
	
	public TestCCAuthentication() throws Throwable { super(); }
	
	@Test
	public void testInit() throws Throwable {
		CCAuthentication.init(x2server);
		//we should be able to pull the system administrator user after initialization
		Field adminUsernameField = CCAuthentication.class.getDeclaredField("USER_ADMIN");
		adminUsernameField.setAccessible(true);
		CCUser adminUser = CCAuthentication.getUser((String)adminUsernameField.get(CCAuthentication.class));
		assertNotNull(adminUser);
		
		//if we want to test this further we could mock the default runtime.
		MockRuntime runtime = new MockRuntime() {
		...
		try {
			...
			/*Field f_created_groups_table = CCAuthentication.class.getDeclaredField("created_groups_table");
			f_created_groups_table.setAccessible(true);
			f_created_groups_table.set(CCAuthentication.class, false);

			Field f_created_users_table = CCAuthentication.class.getDeclaredField("created_users_table");
			f_created_users_table.setAccessible(true);
			f_created_users_table.set(CCAuthentication.class, false);*/

			CCAuthentication.init(x2server);
			adminUser = CCAuthentication.getUser((String)adminUsernameField.get(CCAuthentication.class));
			assertNotNull(adminUser);
			
			//f_created_users_table.set(CCAuthentication.class, false);
			
			Field f_TABLE_USERS = CCAuthentication.class.getDeclaredField("TABLE_USERS");
			f_TABLE_USERS.setAccessible(true);
			runtime.dropNoSQLTable((String)f_TABLE_USERS.get(CCAuthentication.class));
	...
	public void testProcessRequest() throws Throwable {
		...
		HttpServletRequestAdapter requestAdapter = new HttpServletRequestAdapter() {
			...
			public String getRequestURI() {
				return "/x2manager/authentication/user";
			}
		};
		...
		CCAuthentication.processRequest(null, requestAdapter, responseAdapter);
		...
		CCUser standardAuthUser = new CCUser();
		standardAuthUser.setUsername("standarduser@cloudreach.com");
		standardAuthUser.setGroupIds(Arrays.asList(groupAdminId));
		CCAuthentication.processRequest(standardAuthUser, requestAdapter, responseAdapter);
		...
		requestAdapter = new HttpServletRequestAdapter() {
			...
			public byte[] provideData() throws Throwable {
				...
				return CC.toJSONString(user).getBytes("UTF-8");
			}
		};
		CCAuthentication.processRequest(standardAuthUser, requestAdapter, responseAdapter);
		//we are expecting a 400 error because the specified body is invalid
		...
		//ok now we are going to create a user for test purposes x2-test-user@cloudreach.com with the standard user but not associate it to any groups. we will expect an error 400 as an outcome.
		deleteUser("x2-test-user@cloudreach.com");
		requestAdapter = new HttpServletRequestAdapter() {
			@Override
			public String getMethod() {
				return "POST";
			...
			public byte[] provideData() throws Throwable {
				CCUser user = new CCUser();
				user.setUsername("x2-test-user@cloudreach.com");
		...
		try {
			...
			CCUser newUser = CCAuthentication.getUser(user.getUsername());
	...
	protected void deleteUser(String username) throws Throwable {
		HttpServletRequestAdapter requestAdapter = new HttpServletRequestAdapter() {
			...
			public String getParameter(String string) {
				if(string.equals(CCAuthentication.REQ_USER)) {
					return username;
				}
		...
		CCAuthentication.processRequest(getAdminUser(), requestAdapter, response);
		//assertEquals(200,response.getStatus());
	}
	
	protected CCGroup getGroup(String groupId) throws Throwable {
		HttpServletRequestAdapter requestAdapter = new HttpServletRequestAdapter() {
			@Override
			public String getMethod() {
				return "GET";
			...
			@Override
			public String getRequestURI() {
				return "/x2manager/authentication/group";
			}

			@Override
			public String getParameter(String string) {
				if(string.equals(CCAuthentication.REQ_GROUP)) {
					return groupId;
				}
		...
		if(response.getStatus() == 200) {
			return CC.fromJSONString(CCGroup.class, response.getResponseBody());
		}
	...
	protected void deleteGroup(String groupId) throws Throwable {
		...
		CCGroup g = getGroup(groupId);
		assertNull(g);
	}
	
	protected String createGroupWithAdminUser(CCUser user) throws Throwable {
		HttpServletRequestAdapter requestAdapter = new HttpServletRequestAdapter() {
			@Override
			public String getMethod() {
				return "POST";
			...
			@Override
			public byte[] provideData() throws Throwable {
				CCGroup group = new CCGroup("X2 Test Group", "Test Group with "+user.getUsername()+" as administrator of the group", 0, Integer.MAX_VALUE, x2server.getLOCATION());
				group.setAdminUsers(Arrays.asList(user.getId()));
				return CC.toJSONString(group).getBytes("UTF-8");
			}
		};
		HttpServletResponseAdapter response = new HttpServletResponseAdapter() {};
		CCAuthentication.processRequest(getAdminUser(), requestAdapter, response);
		//we expect that the response body will contain the json of the group which has been created.
		CCGroup newGroup = CC.fromJSONString(CCGroup.class, response.getResponseBody());
	...
	protected String getAdminGroupId() throws Throwable {
		...
		CCAuthentication.processRequest(getAdminUser(), request, response);
		CCGroup[] groupList = (CCGroup[])CC.fromJSONString(CC.classOf(new CCGroup[] {}), response.getResponseBody());
		Field groupAdminField = CCAuthentication.class.getDeclaredField("GROUP_ADMIN");
		groupAdminField.setAccessible(true);
		for(CCGroup group : groupList) {
			if(group.getId().equals(Util.md5((String)groupAdminField.get(CCAuthentication.class)))) {
				return group.getId();
			}
		}
	...
	protected CCUser getAdminUser() throws Throwable {
		CCUser adminUser = new CCUser();
